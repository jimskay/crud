...
class ajaxCRUD{
    ...
    function ajaxCRUD($item, $db_table, $db_table_pk, $ajaxcrud_root = "") {
        ...
        $this->emptyTableMessage = "Nenhum registro encontrado. Clique em Adicionar para criar um novo registro.";
	...
	function doAction($action){
		...
		if ($action == 'delete' && $_REQUEST['id'] != ''){
			...
			if ($success){
				$report_msg[] = "$item Removido";
			}
			else{
				$error_msg[] = "$item não removido. Tente novamente.";
			}
		}//action = delete
        ...
        if ($action == 'delete_file' && $_REQUEST['field_name'] && $_REQUEST['id'] != ''){
            ...
            if ($success){
                ...
                $error_msg[] = "Algum erro ocorreu removendo o arquivo. Tente novamente.";
            }
	...
	function showTable(){
        ...
        */
                ...
                foreach($this->display_fields as $field){
                    ...
                    if ( ($this->fieldInArray($field, $this->uneditable_fields) && (!is_numeric($found_category_index))) || !$this->ajax_editing){
                        ...
                        if ($this->fieldInArray($field, $this->file_uploads)){
                            ...
                            if ($cell_data == ''){
                                $table_html .= "<span id='text_" . $field . $id . "'><a style=\"font-size: 9px;\" href=\"javascript:\" onClick=\"document.getElementById('file_$field$id').style.display = ''; document.getElementById('text_$field$id').style.display = 'none'; \">Adicionar Arquivo</a></span> \n";
        ...
        if ($this->add){
            ...
            $add_html .= "  <br /><h3>Novo <b>$item</b></h3>\n";
            ...
            if ($this->ajax_add){
                $add_html .= "<input class=\"editingSize\" type=\"button\" onClick=\"
                    ...
                    return false;\" value=\"Adicionar $item\">";
            }
            else{
	...
	function makeAjaxEditor($unique_id, $field_name, $field_value, $type = 'textarea', $field_size = "", $field_text = "", $onKeyPress_function = ""){
        ...
        <span id=\"" . $prefield ."_edit\" style=\"display: none;\">
            <form style=\"display: inline;\" name=\"form_" . $prefield . "\" id=\"form_" . $prefield . "\" onsubmit=\"
                ...
                function ajax() {
                    ...
                    var req = '/mkt/frm_curriculums/frm_curriculums.php?ajaxAction=update&id=&field=concessionaria&table=vagas&pk=id&val=' + escape(document.getElementById('text_vagasconcessionaria').value);    
            ...
            if ($type == 'text'){
                ...
                $return_html .= "<input ONKEYPRESS=\"$onKeyPress_function\" id=\"text_$prefield\" name=\"$input_name\" type=\"text\" class=\"editingSize editMode\" size=\"$field_size\" value=\"$field_value\" onBlur=\"ajax();\"/>\n";
			}
			else{
        ...
        $return_html .= "
			<input type=\"button\" class=\"editingSize\" value=\"Cancelar\" onClick=\"
    ...
    function makeAjaxDropdown($unique_id, $field_name, $field_value, $dropdown_table, $dropdown_table_pk, $array_list, $selected_dropdown_text = "NOTHING_ENTERED"){
        ...
        $return_html = "<span class=\"editable hand_cursor\" id=\"" . $prefield . "_show\" onClick=\"
			...
			$return_html .= "<input type=\"button\" value=\"Cancelar\" onClick=\"
				document.getElementById('" . $prefield . "_show').style.display = '';
				document.getElementById('" . $prefield . "_edit').style.display = 'none';
			\"/>
		</span>
