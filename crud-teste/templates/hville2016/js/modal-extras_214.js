$j214(function () {
    ...
    */
    window.generateModalTemplate = function (html, className) {
        var $dynamicModal = $j214('<div id="dynamic-modal" class="modal fade">\
    ...
    $body.on('keyup',function(e) {
        if(e.keyCode === 27) {
            $j214('.modal-btn-close').trigger('click');
        }
    ...
    $j214('[data-toggle=modal-html]').on('click', function (e) {
        e.preventDefault();
        var $this = $j214(this),
        contentId = $this.data('id'),
        additionalClass = $this.data('class') || '',
        html = $j214('#' + contentId).clone();
    ...
    */
    $j214('[data-toggle=modal-ajax]').on('click', function (e) {
        e.preventDefault();
        var $this = $j214(this),
        ...
        var $modalContent = $j214('#modal-content')

        
        $modalContent.load(pageURL, function (response, status, xhr) {
            $j214('#dynamic-modal').removeClass('modal-loading');
            if ( status == "error" ) {
    ...
    $j214('[data-toggle=modal-image]').on('click', function (e) {
        e.preventDefault();
        var $this = $j214(this),
        imgSrc = $this.attr('href'),
        imgTitle = $this.attr('title') || '',
        additionalClass = $this.data('class') || '',
        imgHTML = $j214('<img class="modal-image" id="modal-image" />').attr({ src: imgSrc, alt: imgTitle });
        ...
        if($this.attr('data-instagram')){
            ...
            $j214('.modal-content').append(instagramDataHTML);
        ...
        //Check if there is an image caption or image link, and update the imgHTML appropriately
            ...
            $j214('.modal-content').append(captionHTML);
        }
        
        if($j214(this).attr('data-link')){
        ...
        if($j214(this).attr('data-gallery')){
            var galleryName = $this.attr('data-gallery'),
                $galleryElements = $j214('[data-gallery=' + galleryName + ']'),
            ...
            $j214('#dynamic-modal').append("<span class='global-arrow prev-arrow'></span><span class='global-arrow next-arrow'></span>");

            //define function to change gallery image
            var goToNextImage = function(direction){
                var $modalImage = $j214('.modal-image'),
                ...
                if(nextImageCaption != undefined){
                    if($j214('.modal-image-caption-wrapper').length > 0){
                        $j214('.modal-image-caption').text(nextImageCaption);
                    }
                    else{
                    ...
                    if($j214('.modal-image-caption-wrapper').length > 0){
                        $j214('.modal-image-caption-wrapper').remove();
                    }
                }

                if(nextImageLink != undefined){
                    if($j214('.modal-image-link').length > 0){
                        $j214('.modal-image-link').attr('href', nextImageLink);
                    }
                    else{
                        var linkHTML = "<a href='" + nextImageLink + "' class='modal-image-link' target='_blank'></a>";

                        $j214('.modal-image').wrap(linkHTML);
                    }
                }
                else{
                    if($j214('.modal-image-link').length > 0){
                        var $modalImgHTML = $j214('.modal-image-link').contents();

                        $j214('.modal-image-link').replaceWith($modalImgHTML);
                    }
                }
            ...
            $j214('#dynamic-modal .next-arrow').on('click', function(){
                goToNextImage('next');
            });
            $j214('#dynamic-modal .prev-arrow').on('click', function(){
                goToNextImage('prev');
            });
            ...
            //Bind Swipe event
            // $j214('.modal-content').on('swipeleft', function(){
            //     goToNextImage('next');
            // });
            // $j214('.modal-content').on('swiperight', function(){
            //     goToNextImage('prev');
            // });
        }
    });
    ...
    $j214('[data-toggle=modal-video]').on('click', function (e) {
        e.preventDefault();
        var $this = $j214(this),
